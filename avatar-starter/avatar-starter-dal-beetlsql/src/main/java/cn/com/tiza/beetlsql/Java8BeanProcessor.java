...
/**
 * 处理java8中的日期时间映射, LocalDate, LocalTime, LocalDateTime
 */
public class Java8BeanProcessor extends BeanProcessor {

    public Java8BeanProcessor(SQLManager sm) {
        super(sm);
        ...
        handlers.put(LocalDate.class, new JavaSqlTypeHandler() {
            @Override
            public Object getValue(TypeParameter typePara) throws SQLException {
                Date ts = typePara.getRs().getDate(typePara.getIndex());
        ...
        handlers.put(LocalTime.class, new JavaSqlTypeHandler() {
            @Override
            public Object getValue(TypeParameter typePara) throws SQLException {
                Time ts = typePara.getRs().getTime(typePara.getIndex());
