    ...
    case $1 in
        "system")
            for ip in ${SYSTEM[@]};do
                ...
                ssh ${ip} "$ROOT_BIN/$RUN_BIN -start feisi-system prod 8104 -Xms600m -Xmx600m"
            done
            ;;
        "vehicle")
            for ip in ${VEHICLE[@]};do
                echo "start Avatar Vehicle Service ${ip} ..."
                ssh ${ip} "$ROOT_BIN/$RUN_BIN -start feisi-vehicle prod 8100 -Xms1000m -Xmx2000m"
            done
            ;;
        "terminal")
            for ip in ${TERMINAL[@]};do
                echo "start Avatar Terminal Service ${ip} ..."
                ssh ${ip} "$ROOT_BIN/$RUN_BIN -start feisi-terminal prod 8106 -Xms500m -Xmx1000m"
            done
            ;;
        "grampus")
            for ip in ${GRAMPUS[@]};do
                echo "start Avatar Grampus Client ${ip} ..."
                ssh ${ip} "$ROOT_BIN/$RUN_BIN -start feisi-grampus prod 8108 -Xms500m -Xmx1000m"
            done
            ;;
        "gateway")
            for ip in ${GATEWAY[@]};do
                echo "start Avatar Gateway Service ${ip} ..."
                ssh ${ip} "$ROOT_BIN/$RUN_BIN -start feisi-gateway prod 8089 -Xms500m -Xmx500m"
            done
            ;;
        "appserver")
            for ip in ${APPSERVER[@]};do
                echo "start Avatar Appserver Service ${ip} ..."
                ssh ${ip} "$ROOT_BIN/$RUN_BIN -start feisi-appserver prod 8082 -Xms500m -Xmx500m"
            done
            ;;
        "files")
            for ip in ${FILES[@]};do
                echo "start Avatar Files Service ${ip} ..."
                ssh ${ip} "$ROOT_BIN/$RUN_BIN -start feisi-files prod 8110 -Xms500m -Xmx500m"
            done
            ;;
         "message")
             for ip in ${MESSAGE[@]};do
                 echo "start Avatar Message Service ${ip} ..."
                 ssh ${ip} "$ROOT_BIN/$RUN_BIN -start feisi-message prod 8111 -Xms500m -Xmx500m"
             done
             ;;
         "monitor")
             for ip in ${MONITOR[@]};do
                 echo "start Avatar Monitor Service ${ip} ..."
                 ssh ${ip} "$ROOT_BIN/$RUN_BIN -start feisi-monitor prod 8101 -Xms500m -Xmx500m"
             done
             ;;
         "process")
             for ip in ${PROCESS[@]};do
                 echo "start Avatar process Service ${ip} ..."
                 ssh ${ip} "$ROOT_BIN/$RUN_BIN -start feisi-activiti prod 8101 -Xms500m -Xmx500m"
             done
             ;;
