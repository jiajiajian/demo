...
import cn.com.tiza.tstar.common.constant.GlobalConstants;
import cn.com.tiza.tstar.common.utils.DataConverter;
...
public class TStarData {
    ...
    public byte[] toBytes() {
        int leastLen = 19;
        byte[] terminalIDbytes = getTerminalID().getBytes(GlobalConstants.GLOBAL_CHARSET);
        byte[] cmdIdBytes = getCmdID().getBytes(GlobalConstants.GLOBAL_CHARSET);
    ...
    public static TStarData bytes2TStarData(byte[] bytes, int offset, int length) {
        ...
        String terminalID = new String(terminalIDBytes, GlobalConstants.GLOBAL_CHARSET);
        ...
        buffer.get(cmdBytes);
        String cmdID = new String(cmdBytes, GlobalConstants.GLOBAL_CHARSET);
