...
import cn.com.tiza.service.FileService;
...
public class FileController {
    ...
    @PostMapping
    public String create(@RequestBody FileDto file) {
        return fileService.create(file).toString();
    }
    ...
    public Long[] upload(@RequestParam("files") MultipartFile[] files, @RequestParam("names") String[] names,
        ...
        List<FileDto> saveFiles = new ArrayList<>(files.length);
        for (int i = 0; i < files.length; i++) {
            MultipartFile file = files[i];
            try {
                if (file != null && file.getBytes().length > 0) {
                    saveFiles.add(new FileDto(file.getOriginalFilename(), fileType, file.getBytes()));
                }
            } catch (Exception e) {
    ...
    @GetMapping("/{fileId}")
    public FileVM getById(@PathVariable Long fileId) {
        File file = fileDao.single(fileId);
        FileContent content = fileContentDao.single(fileId);
        file.setContent(content);
        return fileMapper.toVM(file);
    }
